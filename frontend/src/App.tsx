...
const getStatusStyles = (status: string) => {
  switch (status?.toUpperCase()) {
    case 'FREE': return { bg: 'bg-[#E1F5EE]', border: 'border-[#006937]', text: 'text-[#006937]', label: 'פנוי' };
...
function App() {
  ...
  const filteredReportRooms = reportRooms.filter((r) => r.room_id.includes(roomQuery.trim()))
  const openReportTab = () => {
    resetReport()
  ...
  const getLogColor = (type: string) => {
    switch (type) {
      case 'success': return '#006937' // Green
      ...
      {!currentUser ? (
        <div className="flex-1 flex flex-col justify-center items-center p-6 max-w-md mx-auto w-full animate-fadeIn mt-12">
          ...
          <div className="text-center space-y-2 mb-8">
            ...
            <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider">אוניברסיטת בר-אילן</p>
          </div>
          ...
          <main className="flex-1 p-4 max-w-md mx-auto w-full h-[calc(100vh-140px)] overflow-y-auto space-y-4 pb-20">
            ...
            {activeTab === 'search' && (
              <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4 animate-fadeIn">
                <h2 className="text-xl font-semibold text-[#004128]">חיפוש וסינון מתקדם</h2>
                <div className="space-y-4 mt-2">
                  
                  {/* ФИЛЬТР ПО ЗДАНИЯМ (ТЕПЕРЬ ДИНАМИЧЕСКИЙ) */}
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase mb-1.5">בחר בניין קמפוס</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-gray-400 uppercase mb-1.5">
                      זמן פנוי מינימלי: <span className="text-[#006937]">{minFreeMinutes} דקות</span>
                    </label>
                    <input 
                      type="range" 
                      min="10" 
                  ...
                  {searchResponse && (
                    <div className="mt-6 border-t border-gray-100 pt-4 animate-fadeIn">
                      <div className="flex justify-between items-center mb-3">
                        <h3 className="text-sm font-semibold text-gray-800">תוצאות חיפוש:</h3>
                        ...
                        {searchResponse.results_count === 0 ? (
                          ...
                          searchResponse.rooms.map((room: any) => (
                            <div key={room.room_id} className="bg-gray-50 border border-gray-100 p-3 rounded-xl flex justify-between items-center shadow-sm">
                              <div>
                                <div className="text-xs text-gray-500 font-semibold uppercase">בניין {room.building_number}</div>
                                <div className="text-base font-semibold text-gray-800">כיתה {room.room_number}</div>
                              </div>
              ...
              <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm text-center space-y-4 animate-fadeIn">
                ...
                <div className="grid grid-cols-4 gap-2 mt-4 pt-4 border-t border-gray-100">
                  <div className="text-center opacity-100">
                    <svg className="w-7 h-7 mx-auto block text-[#006937]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="4" /><path d="M12 2v2M12 20v2M4.9 4.9l1.4 1.4M17.7 17.7l1.4 1.4M2 12h2M20 12h2M4.9 19.1l1.4-1.4M17.7 6.3l1.4-1.4" /></svg>
                    <span className="text-[9px] font-semibold text-gray-400">דיווח ראשון</span>
                  </div>
                  <div className="text-center opacity-100">
                    <svg className="w-7 h-7 mx-auto block text-[#006937]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><circle cx="12" cy="12" r="5" /><circle cx="12" cy="12" r="1.5" /></svg>
                    <span className="text-[9px] font-semibold text-gray-400">דיוק גבוה</span>
                  </div>
                  <div className="text-center opacity-100">
                    <svg className="w-7 h-7 mx-auto block text-[#006937]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3c2 3 5 4 5 8a5 5 0 0 1-10 0c0-1.6.6-2.8 1.5-3.6C8.7 8.4 9 9 10 9c0-2 1-4 2-6z" /></svg>
                    <span className="text-[9px] font-semibold text-gray-400">7 ימים ברצף</span>
                  </div>
            ...
            {activeTab === 'report' && currentUser && (
              <div dir="rtl" className="space-y-4 animate-fadeIn text-right">
                <div className="flex items-center gap-2">
                  <button onClick={() => setActiveTab('map')} className="text-gray-400 hover:text-gray-600" aria-label="חזרה">
                    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 6l6 6-6 6" /></svg>
                  </button>
                  <h2 className="text-xl font-semibold text-[#004128]">דיווח מהיר</h2>
                </div>
                ...
                {reportResult ? (
                    ...
                    <div className="space-y-1.5 relative">
                      <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide">בניין</label>
                      <input
                        type="text"
                        value={buildingQuery}
                        onChange={(e) => { setBuildingQuery(e.target.value); setShowBuildingList(true); setReportBuilding(''); setReportRoomId(null); setRoomQuery(''); }}
                          ...
                          {filteredBuildings.length === 0 ? (
                            ...
                            filteredBuildings.map((b) => (
                              <button
                                key={b}
                                type="button"
                                onClick={() => { setReportBuilding(b); setBuildingQuery(b); setShowBuildingList(false); setReportRoomId(null); setRoomQuery(''); }}
                    ...
                    <div className="space-y-1.5 relative">
                      <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide">כיתה</label>
                      <input
                        type="text"
                        value={roomQuery}
                        disabled={!reportBuilding}
                        onChange={(e) => { setRoomQuery(e.target.value); setShowRoomList(true); setReportRoomId(null); }}
                        onFocus={() => setShowRoomList(true)}
                        placeholder={reportBuilding ? 'הקלד מספר כיתה...' : 'בחר בניין קודם'}
                      ...
                      {showRoomList && reportBuilding && roomQuery.trim() && (
                        <div className="absolute z-10 right-0 left-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-lg max-h-44 overflow-y-auto">
                          {filteredReportRooms.length === 0 ? (
                            <div className="px-4 py-2 text-xs text-gray-400 font-semibold">לא נמצאה כיתה כזו</div>
                          ) : (
                            filteredReportRooms.map((r) => (
                              <button
                                key={r.id}
                                type="button"
                                onClick={() => { setReportRoomId(r.id); setRoomQuery(r.room_id); setShowRoomList(false); }}
                                className="w-full text-right px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-[#E1F5EE] hover:text-[#006937] transition-colors"
                              >
                                כיתה {r.room_id}
                              </button>
                    ...
                    {/* Статус */}
                    <div className="space-y-1.5">
                      <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide">מה המצב עכשיו?</label>
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        >
                          <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><path d="M8.5 12.5l2.5 2.5 4.5-5" /></svg>
                          פנוי
                        </button>
                        <button
                          type="button"
                        ...
                        >
                          <svg className="w-8 h-8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="9" /><path d="M15 9l-6 6M9 9l6 6" /></svg>
                          תפוס
                        </button>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      disabled={reportRoomId == null || !reportStatus || isSubmittingReport}
