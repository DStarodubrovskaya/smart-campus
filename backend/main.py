...
# Global state flags for the Background Simulation Engine
IS_ENGINE_ALIVE = False  
IS_ENGINE_PAUSED = False 
MAX_IDLE_SECONDS = 180 

# --- BACKGROUND PROCESS: SIMULATION ENGINE ---

async def run_simulation_engine():
    """
    Runs the discrete-event simulation (SimPy) alongside the async FastAPI event loop.
    Simulates student movements and triggers the Consensus Algorithm.
    """
    ...
    try:
        ...
        if not users_dict or not valid_rooms:
            print("WARNING: No users or rooms found in the database. Engine stopped.", flush=True)
            return

        def student_agent(env, user_data, all_rooms):
            """Simulates a single user navigating the campus and reporting room statuses."""
            while True:
                yield env.timeout(random.randint(1, 3))

                # 80/20 Rule: Concentrate traffic in hotspots to trigger consensus faster during demos
                ...
                # Agent honesty is probabilistically tied to their Trust Score
                ...
                # Hardcoded demo timeline (Sunday, 10:00 AM, Semester A)
                sim_semester = "א"     
                sim_day = 6          
                sim_time = "10:00:00"  
                ...
                try:
                    # Feed the report into the Consensus Engine
        ...
        # Mount all agents into the SimPy environment
        for uid, user_info in users_dict.items():
            env.process(student_agent(env, user_info, valid_rooms))

        # Bridge SimPy (synchronous discrete time) with AsyncIO (real wall-clock time)
        while IS_ENGINE_ALIVE:
            if not IS_ENGINE_PAUSED:
                ...
                idle_counter += 1
                # Memory leak prevention: Kill zombie processes
...
async def user_login(payload: UserLoginPayload):
    """Handles Mock-Login. Seeds user data if they don't exist in the DB."""
    try:
        response_data = None
...
async def submit_real_user_report(payload: RealUserReport):
    """Processes reports from live frontend users via the Trust Logic Engine."""
    try:
        with db.engine.begin() as conn:
...
def get_ml_model():
    """Lazy-loads the scikit-learn Random Forest artifact to prevent startup crashes if missing."""
